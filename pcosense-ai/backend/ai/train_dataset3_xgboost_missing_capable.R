...
##
## XGBoost on Dataset 3 (32 features), trained the SAME way as the Dataset 1
...
imp <- read.csv("Dataset3_feature_importance.csv")
...
cat("\n=== Held-out test set, Dataset 3, XGBoost native NA routing ===\n")
results <- list()
results$full_21 <- evaluate(X_test, y_test, "All 21 present")

X_mand <- X_test; for (f in OPTIONAL) X_mand[[f]] <- NA
results$mandatory_only <- evaluate(X_mand, y_test, "Only 10 mandatory (11 optional NA)")
...
## ---- save -------------------------------------------------------------------
xgb.save(model, "Dataset3_missing_capable_xgb.model")
saveRDS(list(mandatory = MANDATORY, optional = OPTIONAL, all_features = ALL_FEATURES,
             results = results), "Dataset3_missing_capable_meta.rds")
write.csv(
  data.frame(Scenario = c("All 21 present", "Only 10 mandatory", "~40% optional missing", "~70% optional missing"),
  ...
  "Dataset3_missing_capable_validation.csv", row.names = FALSE
)
cat("\nSaved Dataset3 missing-capable XGBoost model + metadata.\n")
